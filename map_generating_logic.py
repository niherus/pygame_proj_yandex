...
class Decoration:
    ...
    def __init__(self, screen, map, pos, image, mirror=False):
        self.screen = screen
        self.map = map
        self.pos = pos[0], pos[1]
        self.status = image[-5]
        self.image = pygame.image.load(f'decorations\\{image}')
        if mirror:
            self.image = pygame.transform.flip(self.image, mirror, False)
        self.hp = 100

    def draw(self):
        rect = self.image.get_rect()
        rect.center = self.pos[0] + self.map.st_pos[0], self.pos[1] + self.map.st_pos[1]
